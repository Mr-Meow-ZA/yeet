...
function card(game,r){
  if(!r || !Object.keys(r).length){
    return `<article class="card"><span class="pill amber">PREPARING</span><h3 style="margin-top:9px">${game}</h3><p class="sub">Discovery state has not been published for this game yet.</p></article>`;
  }
...
function render(discovery){
  ...
  el.innerHTML=GAME_ORDER.map(game=>card(game,games[game])).join('');
  const meta=$('discoveryMeta');
  if(meta){
    const families=(discovery?.candidate_families||[]).join(' · ');
    meta.innerHTML=discovery?.candidate_count?`Tested <b>${discovery.candidate_count}</b> pre-declared configurations across ${esc(families)}. Selection uses validation data; the held-out period stays untouched until the winner is fixed.`:'Discovery engine is preparing its first candidate search.';
  }
}
