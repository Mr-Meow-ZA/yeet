...
'''
(scripts / 'import_history.py').write_text(importer)

cloud = scripts / 'update_cloud.py'
text = cloud.read_text()
if 'HISTORY_PATH = ROOT / "data" / "historical-results.json"' not in text:
    text = text.replace('STATE_PATH = ROOT / "data" / "cloud-state.json"', 'STATE_PATH = ROOT / "data" / "cloud-state.json"\nHISTORY_PATH = ROOT / "data" / "historical-results.json"')
old = '''def historical_results(state, game):
    rows = [r for r in state.get("results", []) if r.get("game") == game and r.get("date")]
    return sorted(rows, key=lambda r: r["date"], reverse=True)
'''
new = '''def historical_results(state, game):
    rows = [r for r in state.get("results", []) if r.get("game") == game and r.get("date")]
...
'''
if old not in text:
    raise SystemExit('historical_results marker not found')
text = text.replace(old,new)
cloud.write_text(text)
print('installed history importer and cloud integration')
